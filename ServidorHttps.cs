...
class ServidorHttp
{
    ...
    public ServidorHttp(int porta = 8080)
    {
        ...
        try
        {
            ...
            Task servirdorHttpTask = Task.Run(() => AguradarRequests());
            servirdorHttpTask.GetAwaiter().GetResult();
        }
        catch(Exception e)
        {
    ...
    private async Task AguradarRequests()
    {
        while(true)
        {
    ...
    private void ProcessarRequest(Socket conexao, int numeroRequest)
    {
        ...
        if(conexao.Connected)
        {
            ...
            if(textoRequisicao.Length > 0)
            {
                ...
                conexao.Close();
                Console.WriteLine($"\n{bytesEnviados} bytes enviados em resposta á requisição #{numeroRequest}.");
            }
        }
        ...
        string codigoHttp, int qtdeBytes = 0)
    {
        ...
        texto.Append($"Contet-Length: {qtdeBytes}{Environment.NewLine}{Environment.NewLine}");
    ...
    private void CriarHtmlExemplo()
    {
        ...
        html.Append("<meta name=\"viewport\" content=\"width=device-widht, initial-scale=1.0\">");
